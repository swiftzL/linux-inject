 ...
 *
 */
void getCurrentReg(pid_t pid, struct user_regs_struct *regs)
{
...
void dumpreg(pid_t target)
{
	struct user_regs_struct regs;
	getCurrentReg(target, &regs);
	printf("========================dump reg=========================\n");
	printf("rsi %p\n",regs.rsi);
	printf("rdi %p\n",regs.rdi);
	printf("rdx %p\n",regs.rdx);
...
void dumprip(pid_t target)
{
	struct user_regs_struct regs;
	getCurrentReg(target, &regs);
	int size = 20;
	char *newcode = malloc(size);
	ptrace_read(target, regs.rip, newcode, size);
	printf("========================dump rip=========================\n");
...
void ptrace_step(pid_t target)
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		printf("pid %d siginfo:%s\n", target, strsignal(targetsig.si_signo));
		dumpreg(target);
		dumprip(target);
	}
}
...
void checktargetsig(int pid)
{
	...
	// segfault).
	if (targetsig.si_signo != SIGTRAP) // 调试事件导致的停止
	{
